...
public class SLFRecordButton extends View implements View.OnTouchListener {
     ...
     * RecordButton的监听器
     */
    public interface SLFRecordButtonListener {
    ...
    public static final long DELAY_TIME = TOUCH_DELAY_DEFAULT;

    /**
     * 录制时间10s
     */
    private static final long RECORD_TIME_DEFAULT = 10000;
    private long mRecordTime = RECORD_TIME_DEFAULT;

    /**
     * 录制过短时间
     */
    private static final long MIN_RECORD_TIME_DEFAULT = TOUCH_DELAY_DEFAULT+DURING_TIME + 1000;
    private long mMinRecordTime = MIN_RECORD_TIME_DEFAULT;

    /**
     * 缩放系数
     */
    private static final float SCALE_NUM = 0.75f;
    /**
     * 初始状态下内外圆的比例
     */
    private static final float INNER_EXTERNAL = 0.8f;
     ...
     * 触摸事件的监听器
     */
    private SLFRecordButtonListener mRecordButtonListener;
    /**
     * 延时任务
     */
    ...
    public SLFRecordButton(Context context) {
        this(context, null);
    }

    public SLFRecordButton(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SLFRecordButton(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SLFRecordButton);
        mRecordTime = (long) a.getFloat(R.styleable.SLFRecordButton_slf_recordTime, RECORD_TIME_DEFAULT);
        mTouchDelay = (long) a.getFloat(R.styleable.SLFRecordButton_slf_touchDelay, TOUCH_DELAY_DEFAULT);
        mMinRecordTime = (long) a.getFloat(R.styleable.SLFRecordButton_slf_minRecordTime, MIN_RECORD_TIME_DEFAULT)+mTouchDelay+DURING_TIME;
        a.recycle();

        //初始化内部圆的画笔
        mInnerPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mInnerPaint.setColor(getResources().getColor(R.color.white));

        //初始化外圈圆的画笔
        mExternalPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mExternalPaint.setColor(getResources().getColor(R.color.white));
        //初始化弧形画笔
        mProgressPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mProgressPaint.setStyle(Paint.Style.STROKE);
        mProgressPaint.setStrokeWidth(mStrokeWidth);
        mProgressPaint.setColor(getResources().getColor(R.color.teal_200));
    ...
    private void onActionEndAction() {
        ...
        if (mTouchTime < mMinRecordTime) {
            mRecordButtonListener.onLongClickFinish(SLFRecordButton.RECORD_SHORT);
        } else {
            mRecordButtonListener.onLongClickFinish(SLFRecordButton.NORMAL);
        }
        //执行结束动画效果
    ...
    private void startProgressAnimation() {
        ...
        new CountDownTimer(mRecordTime, 50) {

            @Override
            public void onTick(long millisUntilFinished) {

                mSweepAngle = isDone ? 0 : (mRecordTime-millisUntilFinished)/(mRecordTime*1f) * 360;
                //更新ui
                postInvalidate();
                if(isDone){
                    onActionEndAction();
     ...
     */
    public void setRecordButtonListener(SLFRecordButtonListener recordButtonListener) {
