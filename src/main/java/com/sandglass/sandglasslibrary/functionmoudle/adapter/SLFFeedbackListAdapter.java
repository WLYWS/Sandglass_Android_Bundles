...
//        }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        if (holder instanceof NormalHolder) {
            ...
            }else if(datas.get(position).getStatus()==4||datas.get(position).getStatus()==5){
            ...
            ((NormalHolder) holder).slf_feedback_list_item_date.setText(SLFDateFormatUtils.getDateToMyString(context,datas.get(position).getLastReplyTs()));
